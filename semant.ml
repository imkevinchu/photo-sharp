...
let check (globals, functions) =
  ...
  let built_in_decls =
    let add_bind map (name, forms, ret) = StringMap.add name {
      typ = ret;
      fname = name;
      formals = forms;
      locals = []; body = [] } map
    in List.fold_left add_bind StringMap.empty [ ("prints", [(String, "x")], Void);
                                                 ("print", [(Default, "x")], Void);
			                         ("printb", [(Bool, "x")], Void);
			                         ("printf", [(Float, "x")], Void);
			                         ("printbig", [(Int, "x")], Void);
                                                 ("open", [(String, "x")], Image);
  ...
  let check_function func =
    ...
    let rec check_stmt = function
      ...
      | EFor(s1, e1, st) ->
          SEFor(s1, expr e1, check_stmt st)
      | EEFor(s1, e1, e2, st) ->
          SEEFor(s1, expr e1, expr e2, check_stmt st)
